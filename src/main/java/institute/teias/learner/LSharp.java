...
//    private void processCounterExample()
    ...
    private void promoteFrontierState() {
        HashSet<Node<I, O>> newBasis = this.frontierToBasisMap.entrySet().stream()
                .filter(entry -> entry.getValue().isEmpty())
                .map(Map.Entry::getKey)
                .collect(Collectors.toCollection(HashSet::new));

        this.basis.addAll(newBasis);
        this.frontierToBasisMap.keySet().removeAll(newBasis);

        this.frontierToBasisMap.replaceAll((f, basisCandidates) -> {
            HashSet<Node<I, O>> newCandidates = newBasis.stream()
                    .filter(b -> !Apartness.statesAreApart(f, b, this.observationTree))
                    .collect(Collectors.toCollection(HashSet::new));
            basisCandidates.addAll(newCandidates);
